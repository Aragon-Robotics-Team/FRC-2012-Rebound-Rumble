 ...
 **/
bool Gamepad::GetButton(UINT32 button)
{
    return ((0x1 << (button-1)) & ap_ds->GetStickButtons(a_port)) != 0;
}

/**
 * Gets whether or not the left trigger button is pressed.
 * @return The state of the left trigger button.
 */
bool Gamepad::GetLeftTrigger()
{
    return GetButton(kLeftTriggerButton);
}

/**
 * Gets whether or not the right trigger button is pressed.
 * @return The state of the right trigger button.
 */
bool Gamepad::GetRightTrigger()
{
    return GetButton(kRightTriggerButton);
}
